...
import { renderDaytonaUrl, makeOpenInPopup } from '../utils';

namespace Daytonaify {
	export const NAV_BTN_ID = "daytona-btn-nav";
	export const NAV_BTN_CLASS = "daytona-nav-btn";
    export const NAV_BTN_CLASS_SELECTOR = "." + NAV_BTN_CLASS;

    export const EDIT_BTN_ID = "daytona-btn-file";

    export const CSS_REF_BTN_CONTAINER = "daytona-btn-container";
...
export class GitHubInjector extends InjectorBase {
    ...
    checkIsInjected(): boolean {
        const button = document.getElementById(`${Daytonaify.NAV_BTN_ID}`);
        const currentUrl = renderDaytonaUrl(this.config.daytonaURL);
...
abstract class ButtonInjectorBase implements ButtonInjector {
    ...
    inject(currentUrl: string, openAsPopup: boolean) {
        ...
        const oldBtn = document.getElementById(Daytonaify.NAV_BTN_ID);
        if (oldBtn) {
        ...
        // Edit File Menu Options - Open in Daytona
    ...
    protected renderButton(url: string, openAsPopup: boolean): HTMLElement {
        let classes = this.btnClasses + ` ${Daytonaify.NAV_BTN_CLASS}`;
        if (this.float) {
            classes = classes + ` float-right pr-1`;
        }

        const container = document.createElement('div');
        container.id = Daytonaify.CSS_REF_BTN_CONTAINER;
        container.className = classes;

        const a = document.createElement('a');
        a.id = Daytonaify.NAV_BTN_ID;
        a.title = "Daytona";
        a.text = "Daytona"
        a.href = url;
        a.target = "_blank";
    ...
    protected renderEditButton(url: string, openAsPopup: boolean): HTMLElement {
        const a = document.createElement('a');
        a.id = Daytonaify.EDIT_BTN_ID;
        a.title = "Edit this file in Daytona";
        a.text = "Open in Daytona";
        a.href = url;
        a.target = "_blank";
        if (openAsPopup) {
            makeOpenInPopup(a);
...
class FileInjector extends ButtonInjectorBase {
    constructor() {
        super("#StickyHeader > div > div > div:nth-child(2)", "daytona-file-btn");
    }
...
class EmptyRepositoryInjector extends ButtonInjectorBase {
    constructor() {
        super(".repository-content > div > git-clone-help > div > div ", "daytona-file-btn", false, true);
    }
